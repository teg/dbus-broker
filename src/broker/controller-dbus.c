...
static int controller_method_listener_reset_policy(Controller *controller, const char *path, CDVar *in_v, FDList *fds, CDVar *out_v) {
        _c_cleanup_(policy_registry_freep) PolicyRegistry *policy = NULL;
        ControllerListener *listener;
        int r;
        ...
        r = controller_listener_reset_policy(listener, policy);
        if (r)
                return error_trace(r);
        policy = NULL;
...
static int controller_dispatch_listener(Controller *controller, uint32_t serial, const char *method, const char *path, const char *signature, Message *message) {
        static const ControllerMethod methods[] = {
                ...
                { "SetPolicy",        controller_method_listener_reset_policy,        controller_type_in_v,   controller_type_out_unit },
        };
